...
function TablaMovimientos() {
  ...
  const fetchMovimientos = useCallback(async () => {
    try {
      ...
      const response = await fetchAuth(`http://localhost:3000/movimientos?${params.toString()}`);
      ...
      // La API devuelve un objeto con la propiedad data según la convención del proyecto
  ...
  }, [fetchMovimientos, searchParams]); // Depender de searchParams para disparar la carga
      ...
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#eceef0] shadow-sm animate-in fade-in slide-in-from-top-4 duration-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-[#737686] uppercase tracking-wider">Tipo</label>
                <select 
                  name="tipo"
                  value={filters.tipo}
                  onChange={handleFilterChange}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-[#737686] uppercase tracking-wider">Desde</label>
                <input 
                  type="date" 
                  name="desde"
                  value={filters.desde}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-[#737686] uppercase tracking-wider">Hasta</label>
                <input 
                  type="date" 
                  name="hasta"
                  value={filters.hasta}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-white shadow-sm flex items-center gap-4 transition-all hover:shadow-md">
            ...
            <div>
              <p className="text-sm text-[#737686] font-medium">Items Movidos</p>
              <h3 className="text-2xl font-bold text-[#191c1e]">
          ...
          <div className="overflow-x-auto">
                ...
                {movimientos.map((movimiento) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-5">
                      ...
                      {movimiento.notas && (
                        <div className="text-xs text-[#737686] truncate max-w-[200px]" title={movimiento.notas}>
                          {movimiento.notas}
                        </div>
                      )}
                    </td>
            ...
            {!loading && movimientos.length === 0 && (
              <div className="py-24 flex flex-col items-center justify-center text-[#737686] space-y-4">
                ...
                <div className="text-center">
                  <p className="text-xl font-bold text-[#191c1e]">Sin registros</p>
                  <p className="text-sm">No se han encontrado movimientos en el historial.</p>
                </div>
              </div>
            )}
            ...
            {loading && (
              <div className="py-24 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-sm font-medium text-[#737686]">Cargando historial...</p>
              </div>
            )}
          </div>
        </div>
