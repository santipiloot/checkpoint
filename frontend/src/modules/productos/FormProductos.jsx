  ...
  useEffect(() => {
    if (editData && isOpen) {
      setFormData({
        ...
        stock_minimo:
          editData.stock_minimo !== undefined
            ? editData.stock_minimo.toString()
            : "0",
      });
    } else if (isOpen) {
      setFormData({
  ...
  const calcularRop = async () => {
    ...
    try {
      const res = await fetchAuth(
        `http://localhost:3000/productos/${editData.id}/rop`,
      );
      ...
      if (
        ...
        const stockSugerido =
          data.data.stock_minimo_sugerido ||
          data.data.rop_sugerido ||
          data.data;
        setFormData((prev) => ({
          ...prev,
          stock_minimo: stockSugerido.toString(),
        }));
        toast.success(
          `Cálculo Inteligente completado: Sugerido ${stockSugerido}`,
        );
      } else {
        toast.error(
          "No hay suficientes datos de ventas en los últimos 30 días para calcular el ROP.",
        );
      }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  aria-invalid={
                    errores && errores.some((e) => e.path === "codigo_barras")
                  }
                />
                {errores && (
                  <small className="text-red-500 text-xs">
                    {errores
                      .filter((e) => e.path == "codigo_barras")
                      .map((e) => e.msg)
                      .join(", ")}
                  </small>
                )}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  aria-invalid={
                    errores && errores.some((e) => e.path === "precio_costo")
                  }
                />
                {errores && (
                  <small className="text-red-500 text-xs">
                    {errores
                      .filter((e) => e.path == "precio_costo")
                      .map((e) => e.msg)
                      .join(", ")}
                  </small>
                )}
              ...
              {!editData && (
                <div className="space-y-2 md:col-span-2 p-4 bg-orange-50 border border-orange-100 rounded-xl">
                  ...
                  <input
                    ...
                    required
                    aria-invalid={
                      errores && errores.some((e) => e.path === "stock")
                    }
                  />
                  {errores && (
                    <small className="text-red-500 text-xs">
                      {errores
                        .filter((e) => e.path == "stock")
                        .map((e) => e.msg)
                        .join(", ")}
                    </small>
                  )}
                  <p className="text-sm text-orange-700 mt-2">
                    Atención: Solo definible al crear el producto.
                    Modificaciones futuras deben realizarse mediante Movimientos
                    (Ajustes).
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2 md:col-span-2 p-5 bg-indigo-50 border border-indigo-100 rounded-xl shadow-inner relative">
                ...
                <input
                  ...
                  required
                  aria-invalid={
                    errores && errores.some((e) => e.path === "stock_minimo")
                  }
                />
                {errores && (
                  <small className="text-red-500 text-xs">
                    {errores
                      .filter((e) => e.path == "stock_minimo")
                      .map((e) => e.msg)
                      .join(", ")}
                  </small>
                )}
                {editData && (
                  <p className="text-sm text-indigo-700 mt-2 font-medium flex items-center gap-2">
                    <Wand2 className="w-4 h-4 opacity-70" /> Utilice la varita
                    mágica para auto-completar según movimientos reales de los
                    últimos 30 días.
                  </p>
                )}
              </div>
