...
function TablaProductos() {
  ...
  const handleReactivar = async (id) => {
    if (window.confirm("¿Está seguro de reactivar este producto?")) {
      try {
        const response = await fetchAuth(
          `http://localhost:3000/productos/${id}/reactivar`,
          {
            method: "PATCH",
          },
        );
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold shrink-0 ${isCritical ? "bg-red-100 text-red-600" : "bg-[#dbe1ff] text-[#004ac6]"}`}
                          >
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xl font-bold ${isCritical ? "text-red-600" : "text-[#191c1e]"}`}
                          >
                            {producto.stock}
                          </span>
                          {isCritical && (
                            <span
                              title={`ROP Sugerido alcanzado. Min: ${producto.stock_minimo}`}
                            >
                      ...
                      {esAdmin && (
                        <td className="px-4 py-4 text-right font-medium text-[#434655]">
                          {producto.precio_costo
                            ? `$${new Intl.NumberFormat("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(producto.precio_costo)}`
                            : "N/A"}
                        </td>
                      )}
                        ...
                        <div className="flex justify-center items-center gap-2">
                          ...
                          {esAdmin && (
                            <>
                              <button
                                onClick={() =>
                                  handleOpenMovimiento("entrada", producto)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleOpenMovimiento("salida", producto)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleOpenMovimiento("ajuste", producto)
                                }
                                title="Ajuste Manual"
                                disabled={!producto.activo}
